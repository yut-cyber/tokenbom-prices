...
- **KPI 统计卡**：月度用量、当前月度成本、最低/最高成本模型一目了然
- **深浅双主题**：一键切换，记忆偏好，首次访问跟随系统
- **模型能力标签**：上下文长度、视觉、工具调用、推理、PDF、提示缓存、输出速度（tps）
- **筛选与排序**：搜索、状态 tab 筛选、点击表头按任意价格列排序
...
├── src/
│   ├── index.html          # HTML 模板（侧边栏 + 顶栏 + KPI 布局，含 SVG sprite）
│   ├── styles.css          # 设计令牌（Anthropic 风格，含暗色主题）+ 组件样式
